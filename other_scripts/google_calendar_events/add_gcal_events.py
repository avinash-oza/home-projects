...
def read_events_file(file_path):
    ...
    events_df['event_date_start'] = pd.to_datetime(events_df['event_date_start']).dt.tz_localize('America/New_York')
    events_df['event_date_end'] = pd.to_datetime(events_df['event_date_end']).dt.tz_localize('America/New_York')
...
def add_events(events_df, calendar, dry_run):
    for idx, row in events_df.iterrows():
        ...
        logger.info(f"Adding event_name={event_name} with start={start_dt},end={end_dt},location={location}")
...
def main():
    ...
    parser.add_argument('--wipe-calendar', action='store_true', help='Wipe the calendar before adding events')
    parser.add_argument('--dry-run', action='store_true', help="Print out what is being done but don't do it")
    parser.add_argument('--events-file-path', type=str, required=True,
