...
def get_or_create_multipart_upload(file_path, vault_name, part_size_bytes):
    ...
    if pending_uploads['UploadsList']:
        ...
        for one_upload in pending_uploads['UploadList']:
...
def upload_file_to_glacier(file_path, vault_name, part_size=1024, threads=1):
    ...
    for t in range(5):
        ...
        try:
            #TODO: fix the call to this script
            run_output = subprocess.check_call("python glacier-upload/src/glacier_upload/upload.py -v {} -u {}".format(vault_name, upload_id), shell=True)
        except subprocess.CalledProcessError as e:
            logger.error("Retrying upload for {}".format(file_path))
        else:
            logger.info("Successfully uploaded {}".format(file_path))
            break

    #TODO: figure out how to get the upload id here
