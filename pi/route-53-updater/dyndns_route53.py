# SCRIPT COPIED FROM https://gist.github.com/mariocesar/4142563
# Modifications made to the loading of config information
"""
 Requeriments:
...
HOSTED_ZONE = config.get('ROUTE53', 'HOSTED_ZONE')
DOMAIN_NAME = config.get('ROUTE53', 'DOMAIN_NAME')
...
def main():
    ...
    if resolved_ip == current_ip:
        print('DNS response (%s) and public IP (%s) are the same, nothing to do' % (resolved_ip, current_ip))
    ...
    if current_ip not in response.resource_records:
        print('Found new IP: %s' % current_ip)
		...
		sys.exit(2)
                ...
                logger.debug('%s' % change)
        print "WARNING: IP changed to {current_ip}".format(current_ip=current_ip)
        sys.exit(1)
